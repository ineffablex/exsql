...
public class CustomPasswordRetrievalServiceTest {
    ...
    @InjectMocks
    private CustomPasswordRetrievalService customPasswordRetrievalService;
    ...
    void setUp() {
        ...
        ReflectionTestUtils.setField(customPasswordRetrievalService, "cndlAuthFilePath", "test-auth-file-path");
        ...
        ReflectionTestUtils.setField(customPasswordRetrievalService, "primaryCndlGetPasswdUrl", "primaryUrl");
        ReflectionTestUtils.setField(customPasswordRetrievalService, "primaryCndlGetPasswdDummyPassword", "primaryDummy");

        // Call initialize manually as @PostConstruct might not run in this unit test setup without Spring context
        // Or, ensure cndlMgrInitialized is true after mocking init
        mockedCNLDBConnectMgr.when(CNLDBConnectMgr::init).thenAnswer(invocation -> null); // Make init() do nothing
    ...
    void retrievePrimaryPassword_success() throws Exception {
        ...
        mockedCNLDBConnectMgr.when(() -> CNLDBConnectMgr.getPasswd(1, "primaryTns", username, "primaryUrl", "primaryDummy"))
                .thenReturn(expectedPassword);

        String actualPassword = customPasswordRetrievalService.retrievePrimaryPassword(username);

        assertEquals(expectedPassword, actualPassword);
        mockedCNLDBConnectMgr.verify(() -> CNLDBConnectMgr.getPasswd(1, "primaryTns", username, "primaryUrl", "primaryDummy"));
    }
    ...
    void retrievePrimaryPassword_CNLDBThrowsException_rethrowsRuntimeException() {
        ...
        assertTrue(exception.getMessage().contains("Error retrieving password for primary datasource via CNLDBConnectMgr"));
    }
    ...
    void retrievePassword_secondaryDatasource_success() throws Exception {
        ...
        mockedCNLDBConnectMgr.when(() -> CNLDBConnectMgr.getPasswd(nDbType, tns, username, url, dummy))
                .thenReturn(expectedPassword);

        String actualPassword = customPasswordRetrievalService.retrievePassword(username, nDbType, tns, url, dummy, dsName);

        assertEquals(expectedPassword, actualPassword);
        mockedCNLDBConnectMgr.verify(() -> CNLDBConnectMgr.getPasswd(nDbType, tns, username, url, dummy));
    }
    ...
    void retrievePassword_secondaryDatasource_CNLDBThrowsException_rethrowsRuntimeException() {
        ...
        assertTrue(exception.getMessage().contains("Error retrieving password for secondary datasource via CNLDBConnectMgr"));
    }
